...
- `DisplayOptions` can now round-trip through JSON via the new `Serialize(bool indented)` instance method and the two `DisplayOptions.DeserializeAsync` static overloads (string and UTF-8 stream); both deserialize overloads await `EnsureFontLoadedAsync` on every `FigletTextStyle` in `Headers` / `Header` before returning, so the result is render-ready. Polymorphic `IHeaderStyle` is keyed on a `"Kind"` discriminator (`"text"` | `"figlet"`) via the internal `HeaderStyleJsonConverter`; `TextColor` rides on an internal `TextColorJsonConverter` that keeps its constructor private. `RendererTests_TextValidation` now reads an optional `name.json` `DisplayOptions` sidecar next to each `.md` / `.txt` pair, making it easier to add new per-case test fixtures.
