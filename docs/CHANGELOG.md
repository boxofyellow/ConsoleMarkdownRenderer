...
- `DisplayOptions` can now round-trip through JSON via the new `Serialize(JsonSerializerOptions? options = null)` instance method and the two `DisplayOptions.DeserializeAsync` static overloads (string and UTF-8 stream); each accepts an optional `JsonSerializerOptions` that the library copies and augments with the required converters, and both deserialize overloads await `EnsureFontLoadedAsync` on every `FigletTextStyle` in `Headers` / `Header` before returning, so the result is render-ready. Polymorphic `IHeaderStyle` is keyed on a `"$type"` discriminator (`"TextStyle"` | `"FigletTextStyle"`) via the internal `HeaderStyleJsonConverter`; `TextColor` rides on an internal `TextColorJsonConverter` that keeps its constructor private. Both converters are self-sufficient with respect to the caller-supplied options (enums are handled regardless of whether a `JsonStringEnumConverter` is present). `RendererTests_TextValidation` now reads an optional `name.json` `DisplayOptions` sidecar next to each `.md` / `.txt` pair, making it easier to add new per-case test fixtures.
