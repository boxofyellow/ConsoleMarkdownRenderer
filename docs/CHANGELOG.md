...
- `FigletTextStyle.Create` now accepts an optional `fontPath`; the referenced `.flf` file is loaded lazily on first use (or eagerly via `CreateAsync` / `EnsureFontLoadedAsync`). `MarkdownDisplayer.DisplayMarkdownAsync` materializes any `FigletTextStyle` fonts referenced by `DisplayOptions.Headers` / `Header` before rendering. `FigletTextStyle` and `TextColor` now expose public `[JsonConstructor]` constructors so `DisplayOptions` graphs round-trip through `System.Text.Json`. `RendererTests_TextValidation` reads an optional `name.json` `DisplayOptions` sidecar next to each `.md` / `.txt` pair, making it easier to add new per-case test fixtures.
