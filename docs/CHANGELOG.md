...
- `DisplayOptions` can now round-trip through JSON via the new `Serialize(JsonSerializerOptions? options = null)` instance method and the two `DisplayOptions.DeserializeAsync` static overloads (string and UTF-8 stream); each accepts an optional `JsonSerializerOptions` that the library copies and augments with the required converters, and both deserialize overloads await `EnsureFontLoadedAsync` on every `FigletTextStyle` in `Headers` / `Header` before returning, so the result is render-ready. The default JSON shape uses camelCase property names. Polymorphic `IHeaderStyle` is keyed on a required `"$type"` discriminator (`"TextStyle"` | `"FigletTextStyle"`) via the internal `HeaderStyleJsonConverter` — symmetric on read and write, throwing on a missing/unknown discriminator. `TextColor` rides on an internal `TextColorJsonConverter` that keeps its constructor private and emits a compact shape (`{ "named": "…" }` for named colours, `{ "r": …, "g": …, "b": … }` for RGB; no redundant `IsRgb`). Both converters honour the caller-supplied `JsonSerializerOptions` on the write side — `PropertyNamingPolicy` transforms the converter-emitted property names (the literal `"$type"` discriminator is left untouched), `DefaultIgnoreCondition` (`WhenWritingNull` / `WhenWritingDefault`) suppresses null / default values, and enums / nested colours flow back through `JsonSerializer` with the same options so the caller's enum-handling policy applies. Reads remain deliberately lenient (case-insensitive property matching regardless of policy). `RendererTests_TextValidation` now reads an optional `name.json` `DisplayOptions` sidecar next to each `.md` / `.txt` pair, making it easier to add new per-case test fixtures.
